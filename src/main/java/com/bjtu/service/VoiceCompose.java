...
public class VoiceCompose {
    ...
    public boolean getMP3ByText(String text){
        ...
        // cuid
        options.put("cuid", ApplicationEntryPoint.uuid);
        // 语速，取值0-9，默认为5中语速
        options.put("spd", VerifyController.spd);
        // 音调，取值0-9，默认为5中语调
        options.put("pit", VerifyController.pit);
        // 音量，取值0-15，默认为5中音量
        options.put("vol", VerifyController.vol);
        // 发音人选择, 0为女声，1为男声，3为情感合成-度逍遥，4为情感合成-度丫丫，默认为普通女
        options.put("per", VerifyController.per);
